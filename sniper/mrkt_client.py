...
def fetch_saling(token: str, count: int = 20, cursor: str = "",
                  ordering: str | None = None, low_to_high: bool = True) -> dict:
    """Одна страница активных лотов.

    ordering=None -> сортировка по времени выставления (предположительно
    новые сверху — так документация реверс-инжиниринга не уточняет
    направление явно, проверьте на первых реальных оповещениях).
